<script>
	...
	let followings = [];
	const getFollowers = () => {
		Promise.all(user.following.map((following) => getUserById(following.followee))).then((res) => {
			followings = res;
		});
	};
				...
				<div class="hidden sm:flex sm:flex-col sm:items-end">
					<p class="text-xs leading-5 text-gray-500">
						Last seen <time datetime="2023-01-23T13:23Z"
							>{(Date.now() - new Date(following.last_access).getTime()) / 1000}</time
						>
					</p>
				</div>
				<svg
